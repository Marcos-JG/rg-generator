...
import { XML_FIELD_MIME, XML_FIELD_TEXT_PREFIX, beginXmlFieldDrag, endXmlFieldDrag, xmlDataFields } from '../../core/xmlFields';

export default function XmlDataPalette() {
  const { xmlString, xmlFields = [], removeXmlField } = useConfigStore();
  const [search, setSearch] = useState('');
  const fields = useMemo(() => xmlString ? xmlDataFields(xmlString) : [], [xmlString]);
  ...
  return <section className="space-y-3" aria-label="Datos del XML">
    ...
    {!!xmlFields.length && <div className="space-y-2"><h4 className="text-xs font-semibold">Agregados al diseño</h4>{xmlFields.map(field => <div key={field.id} className="flex items-center justify-between text-xs gap-2">
