...
import { XML_FIELD_MIME, XML_FIELD_TEXT_PREFIX, activeXmlFieldDrag, endXmlFieldDrag, xmlDataFields } from '../../core/xmlFields';
import type { XmlField } from '../../types/editor';

export function attachXmlFieldDrop(page: HTMLElement, getXml: () => string, add: (field: XmlField) => void) {
    ...
    const drop = (event: DragEvent) => {
      ...
      if (!xpath) return;
      event.preventDefault(); event.stopPropagation();
      const field = xmlDataFields(getXml() || '').find(field => field.xpath === xpath);
      endXmlFieldDrag();
      if (!field) return;
      ...
      add({ id: `xml-field-${crypto.randomUUID()}`, xpath: field.xpath, label: field.label, x, y });
    };
    doc.addEventListener('dragover', over, true); doc.addEventListener('drop', drop, true);
    return () => { doc.removeEventListener('dragover', over, true); doc.removeEventListener('drop', drop, true); };
}
...
export default function useXmlFieldDrop(ref, active: boolean, _documentKey = 0) {
  useLayoutEffect(() => {
    ...
    return attachXmlFieldDrop(page, () => useConfigStore.getState().xmlString, field => useConfigStore.getState().addXmlField(field));
  });
}
